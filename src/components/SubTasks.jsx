...
const SubTasks = ({ active, subTasks, edit, setNewTasks }) => {
  ...
  const deleteSubtask = (generalTaskId, id) => {
    const newTasks = subTasks.filter((task) => task.id !== id);
  ...
  return (
    <div className="subtasks">
      {subTasks?.map(({ generalTaskId, id, name }) => (
        <div key={id} className="sub_task">
          ...
          {edit ? (
            <div className="edit_buttons">
              <button className="unselect_subtask success">
                <svg
                  width="21"
                  height="21"
                  viewBox="0 0 24 24"
              ...
              <button
                onClick={(e) => {
                  e.preventDefault();
                  console.log('Удалил подзадачу');
                  deleteSubtask(generalTaskId, id)
                  // dispatch(deleteSubTask({ generalTaskId, id, subTasks }))
                }}
